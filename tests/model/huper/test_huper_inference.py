...
def test_huper_inference_ctc_collapse_and_token_filtering():
    ...
    inference = HuPERInference(model=model, processor=processor, device="cpu")
...
def test_huper_inference_resamples_when_sr_mismatch(monkeypatch):
    called = {}
    ...
    monkeypatch.setattr(
        "src.model.huper.huper_inference.torchaudio.functional.resample",
        fake_resample,
    )
...
def test_build_huper_inference_uses_dependencies(monkeypatch):
    called = {}

    class DummyProcessorFactory:
        @staticmethod
        def from_pretrained(hf_repo: str, cache_dir: str | None = None):
            called["processor"] = {"hf_repo": hf_repo, "cache_dir": cache_dir}
            return "processor_obj"

    class DummyModelFactory:
        @staticmethod
        def from_pretrained(hf_repo: str, cache_dir: str | None = None):
            called["model"] = {"hf_repo": hf_repo, "cache_dir": cache_dir}
            return "model_obj"
    ...
    monkeypatch.setattr(huper_builders, "Wav2Vec2Processor", DummyProcessorFactory)
    monkeypatch.setattr(huper_builders, "WavLMForCTC", DummyModelFactory)
    monkeypatch.setattr(huper_builders, "HuPERInference", DummyInference)

    inference = huper_builders.build_huper_inference(
