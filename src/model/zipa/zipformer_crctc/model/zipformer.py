...
class Zipformer2(EncoderInterface):
    ...
    def get_flops_mlp(self, bs, seq):
        ...
        for i, encoder in enumerate(self.encoders):
            mlp = 0.0
            subseq = seq // self.downsampling_factor[i]
            if hasattr(encoder, "layers"):
                for layer in encoder.layers:
                    for para in layer.feed_forward1.parameters():
                        mlp += para.numel()
                    for para in layer.feed_forward2.parameters():
                        mlp += para.numel()
                    for para in layer.feed_forward3.parameters():
                        mlp += para.numel()
            else:
                for layer in encoder.encoder.layers:
                    ...
                    for para in layer.feed_forward3.parameters():
                        mlp += para.numel()
                total_flops += 2 * mlp * bs * subseq
        return total_flops / (10**6)
    ...
    def forward(
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            lengths = (
                x_lens + self.output_downsampling_factor - 1
            ) // self.output_downsampling_factor
        else:
...
class FeedforwardModule(nn.Module):
    ...
    def __init__(self, embed_dim: int, feedforward_dim: int, dropout: FloatLike):
        ...
        self.in_proj = nn.Linear(embed_dim, feedforward_dim, bias=True)
...
class FeedforwardModule(nn.Module):
    ...
    def __init__(self, embed_dim: int, feedforward_dim: int, dropout: FloatLike):
        ...
        # shared_dim=0 means we share the dropout mask along the time axis
        self.mid_proj = ActivationDropoutAndLinear(
            embed_dim,
            feedforward_dim,
            activation="SwooshL",
            dropout_p=dropout,
    ...
    def forward(self, x: Tensor):
        x = self.mid_proj(x)*self.in_proj(x)
        x = self.hidden_balancer(x)
        # mid_proj contains SwooshL activation, then dropout, then linear.
        x = self.out_proj(x)
        x = self.out_whiten(x)
        return x
'''
