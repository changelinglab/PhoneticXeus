...
class AsrModel(nn.Module):
    def __init__(
        ...
        use_ctc_hybrid_head: bool = False,
    ):
        ...
        self.use_ctc_hybrid_head = use_ctc_hybrid_head
        if use_ctc_hybrid_head:
            logging.info("Initializing CTC hybrid head")

            self.ctc_output = nn.Sequential(
                nn.Dropout(p=0.1),
                nn.Linear(encoder_dim, vocab_size - 1),
                # nn.LogSoftmax(dim=-1),
            )
    ...
    def forward_cr_ctc(
        ...
        ctc_output = F.log_softmax(ctc_output, dim=-1)
        if self.use_ctc_hybrid_head:
            blank_logprob = self.blank_pred(encoder_out)  # (2 * N, T, 1)
            scale_prob = torch.clamp(1 - torch.exp(blank_logprob), min=1e-6)
            ctc_output = torch.log(scale_prob) + ctc_output
            ctc_output = torch.cat((blank_logprob, ctc_output), dim=-1).contiguous()
...
def downsample_time_mask(time_mask: torch.Tensor, dtype: torch.dtype):
    ...
    # time_mask = nn.functional.max_pool1d(
    #    time_mask, kernel_size=2, stride=1, padding=0, ceil_mode=True
    # )
